...
def test_check_gt_zero_de_message():
    ...
    assert "muss größer als 0 sein" in str(exc.value)
...
def test_validate_par_dict_period_misaligned():
    ...
    assert "must be an integer multiple of the time step" in str(exc.value)
...
def test_validate_param_single_sbad_chg_max_negative():
    ...
    assert "muss größer als 0" in str(exc.value)
