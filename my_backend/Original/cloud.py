        ...
        try:
            ...
            if REG == "lin":
                try:
                    ...
                    if np.any(lin_prd < 0):
                        print("Warning: Negative predictions found. Adjusting regression.")
                        # Set minimum prediction to 0
                        lin_prd = np.maximum(lin_prd, 0)
...
def interpolate():
    try:
        ...
        df2[y_col] = df2[y_col].replace('[^\d\-\.,]', '', regex=True)
